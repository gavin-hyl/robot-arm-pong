...
#   Single URDF Step
#
#   This captures a single step from one frame to the next.  It is of type:
#
    ...
    def fkin(self, q):
        ...
        for step in self.steps:
            ...
            if step.type is JointType.REVOLUTE:
                ...
                T = T @ step.Tshift @ T_from_Rp(Rotn(step.nlocal, q[step.dof]), np.zeros(3))
            elif step.type is JointType.LINEAR:
                # Linear is a shift followed by a translation:
                T = T @ step.Tshift @ T_from_Rp(Reye(), step.nlocal * q[step.dof])
            else:
                T = T @ step.Tshift
